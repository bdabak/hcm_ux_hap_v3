sap.ui.define(
  [
    "hcm/ux/hapv3/controller/App.controller",
    "sap/ui/core/Control",
  ...
  function (AppController, Control, Controller, JSONModel) {
    ...
    QUnit.module("Initialization", {
      beforeEach: function () {
        ...
        sinon
          .stub(Controller.prototype, "getOwnerComponent")
          .returns(this.oComponentStub);
    ...
    QUnit.test("Should set the control busy without delay", function (assert) {
      ...
      assert.strictEqual(
        oModelData.delay,
        0,
        "The root view has no busy indicator delay set."
      );
      assert.strictEqual(oModelData.busy, true, "The root view is busy.");
    });

    QUnit.test(
      "Should set the control not busy and reset the delay",
      function (assert) {
        var oModelData, oAppController;
        ...
        this.fnMetadataThen = function (fnThenCallback) {
          ...
          assert.strictEqual(
            oModelData.delay,
            this.oViewStub.getBusyIndicatorDelay(),
            "The root view has the default busy indicator delay set."
          );
          assert.strictEqual(
            oModelData.busy,
            false,
            "The root view is not busy."
          );
        }.bind(this);
